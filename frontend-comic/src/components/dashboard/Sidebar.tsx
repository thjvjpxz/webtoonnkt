...
export default function Sidebar({ isSidebarOpen, toggleSidebar }: SidebarProps) {
    ...
    return (
        <div className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-white shadow-md transition-all duration-300 flex flex-col border-r border-green-100 dark:bg-gray-800 dark:border-gray-700`}>
            <div className="p-4 flex items-center justify-between border-b border-green-100 dark:border-gray-700">
                {isSidebarOpen ? (
                    <h1 className="text-xl font-bold text-green-600 dark:text-green-400">Comic Admin</h1>
                ) : (
                    <h1 className="text-xl font-bold text-green-600 dark:text-green-400">CA</h1>
                )}
                <button
