...
public class CommentServiceImpl implements CommentService {
    ...
    @Override
    public BaseResponse<List<CommentResponse>> getCommentsByComic(String comicId, int page, int limit) {
        ValidationUtils.checkNullId(comicId);
        Pageable pageable = PaginationUtils.createPageable(page, limit);
        Page<Comment> comments = commentRepository.findByComicIdAndStatus(comicId, CommentStatus.ACTIVE, pageable);

        List<CommentResponse> commentResponses = comments.getContent().stream()
                .map(comment -> {
                    CommentResponse response = convertToResponse(comment);
                    // Lấy số lượng replies cho mỗi comment
                    long repliesCount = commentRepository.countByParentIdAndStatus(comment.getId(),
    ...
    public BaseResponse<List<CommentResponse>> getCommentsByChapter(String chapterId, int page, int limit) {
        ...
        Pageable pageable = PaginationUtils.createPageable(page, limit);
        Page<Comment> comments = commentRepository.findByChapterIdAndStatus(chapterId, CommentStatus.ACTIVE, pageable);
    ...
    public BaseResponse<List<CommentResponse>> getCommentsByUser(String userId, int page, int limit) {
        ...
        Pageable pageable = PaginationUtils.createPageable(page, limit);
        Page<Comment> comments = commentRepository.findByUserIdAndStatus(userId, CommentStatus.ACTIVE, pageable);
    ...
    public BaseResponse<CommentResponse> getCommentById(String id) {
        ...
        if (comment.getParent() == null) {
            List<Comment> replies = commentRepository.findByParentIdAndStatus(comment.getId(), CommentStatus.ACTIVE);
            List<CommentResponse> replyResponses = replies.stream()
                    .map(this::convertToResponse)
                    .collect(Collectors.toList());
            response.setReplies(replyResponses);
    ...
    @Override
    public BaseResponse<CommentResponse> updateComment(String id, CommentRequest request, String userId) {
        ValidationUtils.checkNullId(id);
        ...
        return BaseResponse.success(response, "Cập nhật bình luận thành công!");
    }
    ...
    @Override
    public BaseResponse<List<CommentResponse>> getLatestCommentsByComic(String comicId, int limit) {
        ValidationUtils.checkNullId(comicId);
        Pageable pageable = PageRequest.of(0, limit, Sort.by(Sort.Direction.DESC, "createdAt"));
        Page<Comment> comments = commentRepository.findLatestCommentsByComic(comicId, CommentStatus.ACTIVE, pageable);

        List<CommentResponse> commentResponses = comments.getContent().stream()
                .map(this::convertToResponse)
                .collect(Collectors.toList());

        return BaseResponse.success(commentResponses);
    }
