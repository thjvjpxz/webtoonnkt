...
import com.thjvjpxx.backend_comic.dto.response.PublisherPersonalStatsResponse.ConversionStats;
...
public class PublisherPersonalStatsServiceImpl implements PublisherPersonalStatsService {
        ...
        public PublisherPersonalStatsResponse getPersonalStats(User publisher) {
                ...
                return PublisherPersonalStatsResponse.builder()
                                ...
                                .conversionStats(buildConversionStats(publisher))
        ...
        private RevenueStats buildRevenueStatsInRange(User publisher, LocalDateTime startDate, LocalDateTime endDate) {
                ...
                return RevenueStats.builder()
                                .totalRevenue(totalRevenue != null ? totalRevenue : 0.0)
                                .monthlyRevenue(totalRevenue != null ? totalRevenue : 0.0) // Trong range được chỉ định
                                .weeklyRevenue(0.0)
                                .dailyRevenue(0.0)
                                .totalPurchases(totalPurchases != null ? totalPurchases : 0L)
                                .build();
        ...
        private ViewFollowStats buildViewFollowStats(User publisher) {
                ...
                Long monthlyViews = comicViewsHistoryRepository.getViewsByPublisherBetweenDates(publisher, startOfMonth,
                                now);
                ...
                return ViewFollowStats.builder()
                                ...
                                .monthlyViews(monthlyViews != null ? monthlyViews : 0L)
                                .totalFollowers(totalFollowers != null ? totalFollowers : 0L)
                                .monthlyFollowers(0L) // Sẽ được tính trong monthlyHistory
        ...
        private ViewFollowStats buildViewFollowStatsInRange(User publisher, LocalDateTime startDate,
                        LocalDateTime endDate) {
                Long totalViews = comicViewsHistoryRepository.getViewsByPublisherBetweenDates(publisher, startDate,
                                endDate);
                Long totalFollowers = userFollowRepository.countTotalFollowersByPublisher(publisher);
                ...
                return ViewFollowStats.builder()
                                .totalViews(totalViews != null ? totalViews : 0L)
                                .monthlyViews(totalViews != null ? totalViews : 0L) // Trong range được chỉ định
                                .totalFollowers(totalFollowers != null ? totalFollowers : 0L)
                                .monthlyFollowers(0L)
                                .monthlyHistory(monthlyHistory)
                                .build();
        ...
        private List<TopComicStats> buildTopComics(User publisher) {
                ...
                for (Object[] row : topByRevenue) {
                        ...
                        TopComicStats existing = comicsMap.get(comic.getId());
                        if (existing != null) {
                                existing.setTotalRevenue(revenue);
                        } else {
                                comicsMap.put(comic.getId(), TopComicStats.builder()
                                                .comicId(comic.getId())
                                                .comicName(comic.getName())
        ...
        private ConversionStats buildConversionStats(User publisher) {
                Long totalViews = comicViewsHistoryRepository.getTotalViewsByPublisher(publisher);
                Long totalPurchases = purchasedChapterRepository.getTotalPurchasesByPublisher(publisher);
                Long uniquePurchasers = purchasedChapterRepository.getUniquePurchasersByPublisher(publisher);
                Double totalRevenue = purchasedChapterRepository.getTotalRevenueByPublisher(publisher);

                totalViews = totalViews != null ? totalViews : 0L;
                totalPurchases = totalPurchases != null ? totalPurchases : 0L;
                uniquePurchasers = uniquePurchasers != null ? uniquePurchasers : 0L;
                totalRevenue = totalRevenue != null ? totalRevenue : 0.0;

                Double conversionRate = totalViews > 0 ? (double) totalPurchases / totalViews * 100 : 0.0;
                Double avgRevenuePerView = totalViews > 0 ? totalRevenue / totalViews : 0.0;

                return ConversionStats.builder()
                                .overallConversionRate(conversionRate)
                                .averageRevenuePerView(avgRevenuePerView)
                                .totalUniqueViewers(totalViews) // Ước tính
                                .totalPurchasers(uniquePurchasers)
                                .build();
        }
